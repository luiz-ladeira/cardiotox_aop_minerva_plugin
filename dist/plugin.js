...
function buildEntityIndex(bioEntities) {
  ...
  bioEntities.forEach(be => {
    const keys = [normalizeName(be.name), ...(be.labels || []).map(normalizeName), ...(be.aliases || []).map(normalizeName)];
    keys.forEach(k => {
      if (k) index[k] = be;
    });
  });
...
async function fetchElementDetails(element, minervaProxy) {
  const url = minervaProxy.project.data.getApiUrls().baseNewApiUrl + "/projects/" + minervaProxy.project.data.getProjectId() + "/models/" + element.modelId + "/bioEntities/elements/" + element.id;
...
function renderUI(container, sheet, bioEntities, minervaProxy) {
  ...
  // ===== Events =====
  $(".access-btn").css("font-weight", "normal").on("click", () => window.open(SPREADSHEET_URL, "_blank"));
  $(".clean-btn").on("click", () => {
...
window.minervaDefine = function () {
  return {
    register: async function (minervaProxy) {
      ...
      try {
        const [sheet, elements] = await Promise.all([fetchSheetData(), fetch(`${minervaProxy.project.data.getApiUrls().baseApiUrl}/projects/${minervaProxy.project.data.getProjectId()}/models/*/bioEntities/elements/`).then(r => r.json())]);
...
},{"../css/styles.css":1,"./minervaAPI":3,"jquery":5}],3:[function(require,module,exports){
/**
